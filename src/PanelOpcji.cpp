...
const wxString OptionsPanel::images [] = {"obraz", "obraz2","obraz3","obraz4"};
...
OptionsPanel::OptionsPanel (wxFrame * parent, wxWindowID ID, wxPoint pos, wxSize size, long style, const wxString &name): wxPanel(parent,ID,pos,size,style,name){
    ...
    _choiceImage = new wxChoice(this,ID_CHOICE2,wxPoint(180,300),wxSize(200,30),4,images,0,wxDefaultValidator,"Choice");
    ...
    if (!MyImage.LoadFile(file, wxBITMAP_TYPE_JPEG ) )
        wxLogError(wxT("Nie mozna zaladowa obrazka, sprawdz, czy w folderze img znajduja sie obrazki"));
    else
...
void OptionsPanel::StartClick(wxCommandEvent& event){
    ...
    if (!MyImage.LoadFile(file, wxBITMAP_TYPE_JPEG ) )
        wxLogError(wxT("Nie mozna zaladowa obrazka " + file + ", sprawdz, czy w folderze img znajduja sie obrazki"));
    else{        
        if(!_buttonContinue)
            ...
            if(ProjektFrame::FRAME->gameStarted()){
                wxMessageDialog a (this,"Na pewno chcesz opuscic uruchomiona gre ?",wxMessageBoxCaptionStr, wxYES_NO|wxICON_QUESTION);
